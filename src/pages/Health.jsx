...
const Health = () => {
  return (
    <div className="bg-black min-h-screen">
      ...
      <h2 className="text-7xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-yellow-400 text-center">
  Revive, Thrive, and Feel Alive
</h2>
<p className="text-lg font-bold mt-4 bg-gradient-to-r from-white to-pink-600 text-transparent bg-clip-text">
  Discover expert tips, actionable advice, and inspiring stories to transform your health and wellness journey.
      ...
      <section className="py-12 px-8 max-w-6xl mx-auto">
        {blogTopics.map((topic, index) => (
          <div 
            key={index} 
            className={`flex flex-col md:flex-row gap-24 items-center mb-16  p-8 rounded-xl transition-transform transform hover:scale-105 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}
          >
            ...
            <div className="md:w-1/2 md:px-2 text-left">
              <h2 className="text-6xl font-extrabold py-4 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 text-transparent bg-clip-text">{topic.title}</h2>
              <p className="text-lg bg-gradient-to-r font-bold from-green-500 to-pink-600 text-transparent bg-clip-text">
  {topic.description}
</p>
              <Link
                to={topic.link}
                className="block mt-6  px-4 mx-20 py-3 bg-gradient-to-r from-gray-400 to-blue-700 text-white text-center text-lg font-semibold rounded-full shadow-xl hover:from-black hover:to-blue-800 transition-all duration-300 transform "
              >
                Read More
              </Link>
            </div>
